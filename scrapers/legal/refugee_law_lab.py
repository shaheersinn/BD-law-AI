...
class RefugeeLawLabScraper(BaseScraper):
    ...
    rate_limit_rps = 0.05; concurrency = 1; ttl_seconds = 604800  # 1 week
    ...
    async def scrape(self) -> list[ScraperResult]:
        ...
        try:
            ...
            results.append(ScraperResult(
                source_id=self.source_id, signal_type="litigation_immigration",
                source_url=data.get("html_url"),
                signal_value={"tag": data.get("tag_name"), "published": data.get("published_at"), "assets": [a.get("name") for a in data.get("assets", [])]},
                signal_text=f"RLL Dataset Release: {data.get('tag_name')}",
                published_at=self._parse_date(data.get("published_at")),
                practice_area_hints=["immigration"], raw_payload={"tag": data.get("tag_name")},
            ))
