...
class HealthCanadaScraper(BaseScraper):
    ...
    rate_limit_rps = 0.1; concurrency = 1; ttl_seconds = 43200

    async def scrape(self) -> list[ScraperResult]:
        results = []
        try:
            resp = await self.get("https://healthycanadians.gc.ca/recall-alert-rappel-avis/api/recent/en")
            if resp.status_code == 200:
                ...
                for item in data.get("results", {}).get("ALL", [])[:50]:
                    ...
                    results.append(ScraperResult(
                        source_id=self.source_id, signal_type="regulatory_health_recall",
                        raw_company_name=company or None,
                        source_url=item.get("url"),
                        signal_value={"title": title, "category": item.get("category"), "date": item.get("date_published")},
                        signal_text=f"Health Canada: {title}" + (f" — {company}" if company else ""),
