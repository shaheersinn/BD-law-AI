...
class Settings(BaseSettings):
    ...
    db_pool_recycle: int = Field(
        default=3600, description="DB connection recycle time in seconds"
    )
    ...
    mongodb_max_pool_size: int = Field(
        default=50, description="MongoDB connection pool size"
    )
    ...
    redis_rate_limit_ttl: int = Field(
    ...
    celery_task_time_limit: int = Field(
        default=3600, description="Hard task time limit in seconds"
    )
    ...
    spaces_secret: str = Field(
        default="", description="DigitalOcean Spaces secret key"
    )
    spaces_bucket: str = Field(
        default="oracle-models", description="Spaces bucket name"
    )
    ...
    proxycurl_api_key: str = Field(
        default="", description="Proxycurl API key (LinkedIn)"
    )
    ...
    slack_webhook_url: str = Field(
        default="", description="Slack webhook for critical alerts"
    )

    # ── Rate Limiting ──────────────────────────────────────────────────────────
    rate_limit_requests: int = Field(
        default=100, description="Max requests per window per user"
    )
    ...
    def validate_production_secrets(self) -> "Settings":
        ...
        if self.environment == "production":
            ...
            if self.database_url.startswith("postgresql+asyncpg://oracle:oracle@localhost"):
                raise ValueError(
                    "DATABASE_URL must be set to a real database URL in production"
                )
        return self
