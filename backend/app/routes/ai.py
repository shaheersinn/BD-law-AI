...
from app.cache.client import TTL_AI, TTL_GEO, TTL_LONG, cache
...
async def regulatory_alert(
    ...
    draft = await ai.generate(
        "regulatory_alert",
        source=req.source, title=req.title, date=req.date,
        practice_area=req.practice_area,
        client_name=client.name, industry=client.industry, region=client.region,
    ...
    await log_event(
        ...
        resource_type="client", resource_id=req.client_id,
...
async def prospect_outreach(
    ...
    strategy = await ai.generate(
        "prospect_outreach",
        name=req.name, score=req.score, need=req.need, warmth=req.warmth,
        value=req.value, window=req.window,
...
async def prospect_outreach_stream(
    name: str, score: int, need: str, warmth: str, value: str, window: str,
    ...
    return StreamingResponse(
        stream_ai_response(
            "prospect_outreach",
            name=name, score=score, need=need, warmth=warmth,
...
async def alumni_message(
    ...
    message = await ai.generate(
        ...
        name=person.name, role=person.current_role, company=person.current_company,
    ...
    await log_event(
        AuditEventType.ai_generate, user_id=claims.user_id,
        resource_type="alumni", resource_id=alumni_id,
...
async def gc_profile(
    ...
    try:
        ...
        profile = {
            ...
            "key_concerns": [],
            "pitch_hooks": [],
            "credibility": 0, "reliability": 0, "intimacy": 0,
            "self_orientation": 0, "trust_score": 0,
        }
...
async def mandate_brief(
    ...
    brief = await ai.generate(
        "mandate_brief",
        company=req.company, window=req.window,
        signals=signals_text, confidence=req.confidence,
        practice=req.practice, value=req.value,
    )
...
async def mandate_brief_stream(
    company: str, confidence: int, practice: str, value: str, window: str, signals: str,
    claims: TokenClaims = Depends(require_auth),
    request: Request = None,
):
    await enforce_rate_limit(request, "ai", str(claims.user_id))
...
async def ma_strategy(
    ...
    strategy = await ai.generate(
        "ma_strategy",
        company=req.company, deal_type=req.deal_type, value=req.value,
        days=req.days, confidence=req.confidence, warmth=req.warmth,
        signals="\n".join(f"- {s}" for s in req.signals),
    )
...
async def coaching_brief(
    ...
    if not claims.is_admin and claims.partner_id and claims.partner_id != partner_id:
        raise HTTPException(status_code=403, detail="Partners can only access their own coaching brief")
    ...
    stale_result = await db.execute(
        ...
        ).order_by(ReferralContact.revenue_sent.desc()).limit(3)
    )
    stale = stale_result.scalars().all()
    stale_text = "; ".join(
        f"{r.contact_name} at {r.firm_name} — "
        f"{(datetime.now(timezone.utc) - r.last_contact).days}d silent, "
    ...
    cp_result = await db.execute(
        select(ContentPiece).where(ContentPiece.partner_id == partner_id)
        .order_by(ContentPiece.published_at.desc()).limit(1)
    )
    cp = cp_result.scalars().first()
    last_content_days = (
        (datetime.now(timezone.utc) - cp.published_at).days
        if cp and cp.published_at else 999
    )
...
async def ghost_draft(
    ...
    cache_key = cache.ai_response_key("linkedin_draft", partner=req.partner_id, topic=req.topic[:60])
    cached = await cache.get(cache_key)
    if cached:
        return cached
    ...
    samples_text = "\n\n---\n\n".join(s.text for s in samples) if samples else (
    ...
    draft = await ai.generate(
        "linkedin_draft",
        name=partner.name, title=partner.title or "Partner",
    ...
    await log_event(
        AuditEventType.ai_generate, user_id=claims.user_id,
        resource_type="partner", resource_id=req.partner_id,
