...
class ModelPerformanceOut(BaseModel):
    ...
    precision_at_high: float | None
    recall: float | None
    avg_advance_days: float | None
...
class SystemHealthOut(BaseModel):
    ...
    redis: str
    celery: str
    last_sedar_scrape: datetime | None
    last_edgar_scrape: datetime | None
...
async def model_performance(
    ...
    alerts = (await db.execute(select(Alert).where(Alert.fired_at >= cutoff))).scalars().all()

    total = len(alerts)
    labelled = sum(1 for a in alerts if a.confirmed is not None)
    confirmed = sum(1 for a in alerts if a.confirmed)
    dismissed = sum(1 for a in alerts if a.dismissed)
    matter_opened = sum(1 for a in alerts if a.matter_opened)
    ...
    precision_high = None
    if high_alerts:
        tp = sum(1 for a in high_alerts if a.confirmed)
        ...
        (
            await db.execute(
                select(Trigger).where(
                    and_(Trigger.filed_at >= cutoff, Trigger.confirmed.isnot(None))
                )
            )
        )
        .scalars()
    ...
    best = max(
        source_stats,
        key=lambda s: source_stats[s]["confirmed"] / max(source_stats[s]["total"], 1),
        default=None,
    )
    worst = min(
        source_stats,
...
async def signal_quality(
    ...
    result = await db.execute(
        select(
            ...
            func.sum(func.cast(Trigger.confirmed, type_=type(func.count()))).label("confirmed"),
            func.sum(func.cast(Trigger.dismissed, type_=type(func.count()))).label("dismissed"),
...
async def bd_performance(
    ...
    fast_meetings = [m for m in meetings if m.had_followup_within_48h]
    fast_win_rate = None
    if fast_meetings:
        won = sum(1 for m in fast_meetings if m.led_to_matter)
        fast_win_rate = round(won / len(fast_meetings) * 100, 1)

    slow_meetings = [m for m in meetings if not m.had_followup_within_48h]
    slow_win_rate = None
    if slow_meetings:
        won = sum(1 for m in slow_meetings if m.led_to_matter)
    ...
    return {
        ...
        "followup_speed_advantage": (
            ...
            if fast_win_rate is not None and slow_win_rate is not None
            else None
        ),
...
async def system_health(
    ...
    last_sedar = (
        await db.execute(select(func.max(Trigger.detected_at)).where(Trigger.source == "SEDAR"))
    ).scalar()

    last_edgar = (
        await db.execute(select(func.max(Trigger.detected_at)).where(Trigger.source == "EDGAR"))
    ).scalar()

    last_alert = (await db.execute(select(func.max(Alert.fired_at)))).scalar()

    triggers_24h = (
        await db.execute(select(func.count(Trigger.id)).where(Trigger.detected_at >= cutoff_24h))
    ).scalar() or 0

    alerts_24h = (
        await db.execute(select(func.count(Alert.id)).where(Alert.fired_at >= cutoff_24h))
    ).scalar() or 0
