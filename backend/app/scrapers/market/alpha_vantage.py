...
class AlphaVantageScraper(BaseScraper):
    source_id = "market_alpha_vantage"
        ...
        for company in companies[:5]:  # Max 5 lookups per run (budget)
            ...
            signals.append(
                SignalData(
                    ...
                    raw_company_name=company.get("name", ticker),
                    signal_text=f"Significant price movement: {company.get('name', ticker)} {change_pct:+.1f}%",
                    source_url=f"https://finance.yahoo.com/quote/{ticker}",
                    practice_area_hints=["securities_capital_markets"],
                    confidence_score=min(0.85, 0.50 + abs(change_pct) / 20),
                    signal_value={
                        "ticker": ticker,
