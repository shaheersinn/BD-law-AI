"""Canadian Bar Association — class action section publications and trend intelligence."""
...
_BASE_URL = "https://www.cba.org/Sections/Class-Actions"
_STALE_DAYS = 90
...
_KEYWORD_PA_MAP: dict[str, str] = {
    ...
    "mining": "mining_natural_resources",
    "franchise": "franchise_distribution",
    "technology": "technology_fintech_regulatory",
}
...
def _extract_company_name(title: str) -> str | None:
    """Try to extract the defendant company name from a case title or article."""
    ...
    async def scrape(self) -> list[ScraperResult]:
        ...
        try:
            ...
            articles = soup.select(
                "article, .publication-item, .news-item, .content-listing a, .list-item"
            )
            if not articles:
                articles = soup.select("a[href*='Article'], a[href*='article']")
            ...
            for item in articles[:40]:
                ...
                results.append(
                    ScraperResult(
                        ...
                        source_url=url,
                        signal_value={"title": title, "source": "CBA Class Actions Section"},
            ...
            log.error("cba_class_actions_scrape_error", error=str(exc), exc_info=True)
        return results
