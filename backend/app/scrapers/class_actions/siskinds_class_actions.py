"""
app/scrapers/class_actions/siskinds_class_actions.py — Siskinds LLP class actions.

Data source: https://www.siskinds.com/class-actions/
Approach: HTML scraping with BeautifulSoup
Signal value: HIGH — Siskinds is a leading Canadian class action firm
"""
...
_BASE_URL = "https://www.siskinds.com"
_CLASS_ACTIONS_URL = f"{_BASE_URL}/class-actions/"


@register
class SiskindsClassActionsScraper(BaseScraper):
    """
    Siskinds LLP class actions scraper.

    Siskinds is one of Canada's leading class action firms.
    Scrapes active investigations and current class proceedings.
    Rate limit: 0.5 rps (commercial law firm site).
    """

    source_id = "class_action_siskinds"
    source_name = "Siskinds LLP — Class Actions"
    signal_types = ["class_action_investigation", "class_action_filed"]
    ...
    async def scrape(self) -> list[ScraperResult]:
        ...
        try:
            ...
            if resp.status_code != 200:
                log.warning("siskinds_fetch_failed", status=resp.status_code)
                return results
            ...
            items = (
                ...
                or soup.select("ul li a[href*='class-action']")
            ...
            log.error("siskinds_scrape_error", error=str(exc))

        log.info("siskinds_scrape_complete", count=len(results))
        return results
    ...
    def _parse_item(
        ...
        return ScraperResult(
            ...
            signal_text=f"Siskinds class action: {title}",
            published_at=published_at or self._now_utc(),
            practice_area_hints=practice_areas,
            raw_payload={"title": title, "firm": "Siskinds LLP", "url": source_url},
    ...
    def _classify_signal(title: str) -> str:
        lower = title.lower()
        if "investigation" in lower or "investigating" in lower or "inquiry" in lower:
    ...
    def _extract_company(title: str) -> str | None:
        for sep in [" v. ", " vs. ", " against ", " re "]:
            if sep in title.lower():
                ...
                name = title[idx:].split(",")[0].split("(")[0].split(" — ")[0].strip()
    ...
    def _infer_practice_areas(title: str) -> list[str]:
        ...
        if "product" in lower or "recall" in lower:
        ...
        if "employ" in lower or "wage" in lower:
