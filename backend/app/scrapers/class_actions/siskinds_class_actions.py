"""Siskinds LLP — leading Canadian class action firm."""
...
_BASE_URL = "https://www.siskinds.com/class-actions/"
_STALE_DAYS = 90
...
def _infer_practice_areas(text: str) -> list[str]:
    ...
    for kw, pa in _KEYWORD_PA_MAP.items():
        if kw in lower:
            areas.add(pa)
    if "securities" in lower or "investor" in lower:
...
class SiskindsClassActionsScraper(BaseScraper):
    source_id = "class_actions_siskinds"
    ...
    async def scrape(self) -> list[ScraperResult]:
        ...
        try:
            ...
            articles = soup.select("article, .class-action-item, .entry, .post")
            if not articles:
            ...
            for item in articles[:60]:
                ...
                if url and not url.startswith("http"):
                    url = f"https://www.siskinds.com{url}"
                ...
                signal_type = (
                    "class_action_filed"
                    if any(w in title.lower() for w in ("filed", "commenced", "certified"))
                ...
                results.append(
                    ScraperResult(
                        ...
                        source_url=url,
                        signal_value={"title": title, "firm": "Siskinds LLP"},
            ...
            log.error("siskinds_scrape_error", error=str(exc), exc_info=True)
        return results
