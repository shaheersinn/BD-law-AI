"""
app/scrapers/class_actions/branch_macmaster_class_actions.py — Branch MacMaster LLP.

Data source: https://www.branmac.com/class-actions/
Approach: HTML scraping with BeautifulSoup
Signal value: HIGH — Major BC class action firm
"""
...
_BASE_URL = "https://www.branmac.com"
...
class BranchMacMasterClassActionsScraper(BaseScraper):
    """
    Branch MacMaster LLP class actions scraper.

    Major BC class action firm. Scrapes active investigations.
    Rate limit: 0.5 rps (commercial law firm site).
    """

    source_id = "class_action_branch_macmaster"
    ...
    async def scrape(self) -> list[ScraperResult]:
        ...
        try:
            ...
            if resp.status_code != 200:
                log.warning("branch_macmaster_fetch_failed", status=resp.status_code)
                return results
            ...
            log.error("branch_macmaster_scrape_error", error=str(exc))

        log.info("branch_macmaster_scrape_complete", count=len(results))
        return results
    ...
    def _parse_item(
        ...
        return ScraperResult(
            ...
            signal_value={
                "title": title,
                "firm": "Branch MacMaster LLP",
                "case_type": "class_action",
                "jurisdiction": "BC",
            },
            signal_text=f"Branch MacMaster investigation: {title}",
            published_at=published_at or self._now_utc(),
            practice_area_hints=["class_actions", "litigation"],
            raw_payload={"title": title, "firm": "Branch MacMaster LLP", "url": source_url},
