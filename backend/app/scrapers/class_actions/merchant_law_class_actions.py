"""Merchant Law Group LLP — national class action firm."""
...
_BASE_URL = "https://www.merchantlaw.com/class-actions"
_STALE_DAYS = 90
...
class MerchantLawClassActionsScraper(BaseScraper):
    source_id = "class_actions_merchant_law"
    source_name = "Merchant Law Group Class Actions"
        ...
        try:
            ...
            for item in articles[:50]:
                ...
                if url and not url.startswith("http"):
                    url = f"https://www.merchantlaw.com{url}"
                ...
                results.append(
                    ScraperResult(
                        ...
                        source_url=url,
                        signal_value={"title": title, "firm": "Merchant Law Group LLP"},
                        signal_text=f"Merchant Law class action: {title}",
            ...
            log.error("merchant_law_scrape_error", error=str(exc), exc_info=True)
        return results
