...
_BASE_URL = "https://www.ontariocourts.ca/scj/class-actions/"
...
_STATUS_SIGNAL_MAP: dict[str, str] = {
    "certified": "class_action_certified",
    "settled": "class_action_settled",
    "approved": "class_action_settled",
}
...
class OntarioClassProceedingsScraper(BaseScraper):
    source_id = "class_actions_ontario"
    source_name = "Ontario Superior Court of Justice — Class Proceedings"
    ...
    async def scrape(self) -> list[ScraperResult]:
        ...
        try:
            ...
            if resp.status_code != 200:
                log.warning("ontario_class_non200", status=resp.status_code)
                return results

            soup = BeautifulSoup(resp.text, "lxml")
            cutoff = datetime.now(tz=UTC) - timedelta(days=90)

            rows = soup.select("table tr, li, .view-content .views-row")
            for row in rows[:80]:
                try:
                    ...
                    log.debug("ontario_class_row_skip", error=str(exc))
                    continue

            await self._rate_limit_sleep()
        except Exception as exc:
            log.error("ontario_class_error", error=str(exc))
        return results
    ...
    def _parse_row(
        ...
        if link_tag and link_tag.get("href"):
            ...
            source_url = href if href.startswith("http") else f"https://www.ontariocourts.ca{href}"
        ...
        signal_type = self._infer_signal_type(text)
        hints = self._infer_practice_areas(text)

        court_file = ""
        file_match = re.search(r"(?:CV|No\.?)\s*[\-:]?\s*([\w\-/]+)", text)
        if file_match:
            court_file = file_match.group(1)
        ...
        return ScraperResult(
            ...
            signal_text=f"Ontario class proceeding: {case_title}",
    ...
    def _infer_signal_type(text: str) -> str:
        lower = text.lower()
        for keyword, sig in _STATUS_SIGNAL_MAP.items():
            if keyword in lower:
                return sig
