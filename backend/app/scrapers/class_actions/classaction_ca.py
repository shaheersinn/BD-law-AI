"""
app/scrapers/class_actions/classaction_ca.py — ClassAction.org Canadian section aggregator.

Data source: https://www.classaction.org/canada
Approach: HTML scraping with BeautifulSoup
Signal value: MEDIUM — aggregator captures investigations before official filings
"""

from __future__ import annotations

from datetime import UTC, datetime, timedelta
from typing import Any

import structlog

from app.scrapers.base import BaseScraper, ScraperResult
from app.scrapers.registry import register

log = structlog.get_logger(__name__)

_BASE_URL = "https://www.classaction.org"
_CANADA_URL = f"{_BASE_URL}/canada"


@register
class ClassActionOrgScraper(BaseScraper):
    """
    ClassAction.org Canadian section scraper.

    Aggregator that captures class action news and investigations
    before official court filings. Rate limit: 0.5 rps (commercial site).
    """

    source_id = "class_action_org"
    source_name = "ClassAction.org (Canada)"
    signal_types = ["class_action_news", "class_action_investigation"]
    CATEGORY = "class_actions"
    rate_limit_rps = 0.5
    concurrency = 1
    requires_auth = False

    async def scrape(self) -> list[ScraperResult]:
        results: list[ScraperResult] = []
        cutoff = datetime.now(tz=UTC) - timedelta(days=90)

        try:
            resp = await self.get(_CANADA_URL)
            if resp.status_code != 200:
                log.warning("classaction_org_fetch_failed", status=resp.status_code)
                return results

            from bs4 import BeautifulSoup

            soup = BeautifulSoup(resp.text, "html.parser")

            items = (
                soup.select("article")
                or soup.select("div.node--type-article")
                or soup.select("div.views-row")
                or soup.select("div.card, li.list-group-item")
            )

            for item in items[:30]:
                result = self._parse_article(item, cutoff)
                if result:
                    results.append(result)

        except Exception as exc:
            log.error("classaction_org_scrape_error", error=str(exc))

        log.info("classaction_org_scrape_complete", count=len(results))
        return results

    def _parse_article(
        self, item: Any, cutoff: datetime
    ) -> ScraperResult | None:
        title_el = item.find(["h2", "h3", "h4", "a"])
        if not title_el:
            return None
        title = self.safe_text(title_el)
        if not title or len(title) < 10:
            return None

        link_el = item.find("a", href=True)
        source_url = None
        if link_el:
            href = str(link_el.get("href", ""))
            source_url = href if href.startswith("http") else f"{_BASE_URL}{href}"

        date_el = item.find("time") or item.find(
            class_=lambda c: c and "date" in str(c).lower()
        )
        published_at = None
        if date_el:
            published_at = self._parse_date(
                date_el.get("datetime") or self.safe_text(date_el)
            )

        if published_at and published_at < cutoff:
            return None

        summary_el = item.find("p") or item.find(class_="summary")
        summary = self.safe_text(summary_el) if summary_el else ""

        signal_type = self._classify_signal(title, summary)
        company = self._extract_company(title)
        practice_areas = self._infer_practice_areas(title, summary)

        return ScraperResult(
            source_id=self.source_id,
            signal_type=signal_type,
            raw_company_name=company,
            source_url=source_url,
            signal_value={
                "title": title,
                "summary": summary[:500],
                "case_type": "class_action",
                "source": "classaction.org",
            },
            signal_text=f"ClassAction.org: {title}",
            published_at=published_at or self._now_utc(),
            practice_area_hints=practice_areas,
            raw_payload={"title": title, "summary": summary, "url": source_url},
            confidence_score=0.70,
        )

    @staticmethod
    def _classify_signal(title: str, summary: str) -> str:
        combined = f"{title} {summary}".lower()
        if "investigation" in combined or "investigating" in combined:
            return "class_action_investigation"
        if "filed" in combined or "lawsuit" in combined:
            return "class_action_news"
        return "class_action_news"

    @staticmethod
    def _extract_company(title: str) -> str | None:
        for sep in [" against ", " vs. ", " v. ", " sues "]:
            if sep in title.lower():
                idx = title.lower().index(sep) + len(sep)
                name = title[idx:].split(" for ")[0].split(" over ")[0].split(",")[0].strip()
                if name and len(name) > 2:
                    return name
        return None

    @staticmethod
    def _infer_practice_areas(title: str, summary: str) -> list[str]:
        areas: list[str] = ["class_actions", "litigation"]
        combined = f"{title} {summary}".lower()
        mapping = {
            "securities": ["securities_capital_markets"],
            "product": ["product_liability"],
            "privacy": ["privacy_cybersecurity", "data_privacy_technology"],
            "data breach": ["privacy_cybersecurity", "data_privacy_technology"],
            "employ": ["employment_labour"],
            "environ": ["environmental_indigenous_energy"],
            "price fix": ["competition_antitrust"],
            "antitrust": ["competition_antitrust"],
            "insur": ["insurance_reinsurance"],
            "consumer": ["litigation"],
        }
        for keyword, hints in mapping.items():
            if keyword in combined:
                areas.extend(hints)
        return list(dict.fromkeys(areas))
