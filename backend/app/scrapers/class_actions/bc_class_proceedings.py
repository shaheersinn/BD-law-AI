...
_BASE_URL = "https://www.bccourts.ca/supreme_court/class_actions/"
...
class BCClassProceedingsScraper(BaseScraper):
    source_id = "class_actions_bc"
    source_name = "BC Supreme Court — Class Proceedings Registry"
    ...
    async def scrape(self) -> list[ScraperResult]:
        ...
        try:
            ...
            if resp.status_code != 200:
                log.warning("bc_class_non200", status=resp.status_code)
                return results

            soup = BeautifulSoup(resp.text, "lxml")
            cutoff = datetime.now(tz=UTC) - timedelta(days=90)

            rows = soup.select("table tr, .content-area li, .view-content .views-row")
            for row in rows[:80]:
                try:
                    ...
                    log.debug("bc_class_row_skip", error=str(exc))
                    continue

            await self._rate_limit_sleep()
        except Exception as exc:
            log.error("bc_class_error", error=str(exc))
        return results
    ...
    def _parse_row(
        ...
        if link_tag and link_tag.get("href"):
            ...
            source_url = href if href.startswith("http") else f"https://www.bccourts.ca{href}"
        ...
        signal_type = "class_action_certified" if "certified" in text.lower() else "class_action_filed"
        hints = self._infer_practice_areas(text)

        court_file = ""
        file_match = re.search(r"(?:No\.?|S)\s*[\-:]?\s*([\w\-/]+)", text)
        if file_match:
        ...
        return ScraperResult(
            ...
            signal_text=f"BC class proceeding: {case_title}",
