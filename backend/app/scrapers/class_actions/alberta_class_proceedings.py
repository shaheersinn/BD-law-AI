...
_BASE_URL = "https://www.albertacourts.ca/kb/class-proceedings"
...
_KEYWORD_PA_MAP: dict[str, str] = {
    ...
    "oil": "environmental_indigenous_energy",
    "pipeline": "environmental_indigenous_energy",
    ...
    "energy": "environmental_indigenous_energy",
}
...
class AlbertaClassProceedingsScraper(BaseScraper):
    source_id = "class_actions_alberta"
    ...
    async def scrape(self) -> list[ScraperResult]:
        ...
        try:
            ...
            if resp.status_code != 200:
                log.warning("alberta_class_non200", status=resp.status_code)
                return results

            soup = BeautifulSoup(resp.text, "lxml")
            cutoff = datetime.now(tz=UTC) - timedelta(days=90)

            rows = soup.select("table tr, .content-area li, .views-row")
            for row in rows[:80]:
                try:
                    ...
                    log.debug("alberta_class_row_skip", error=str(exc))
                    continue
    ...
    def _parse_row(self, row: Tag, cutoff: datetime) -> ScraperResult | None:
        ...
        if link_tag and link_tag.get("href"):
            ...
            source_url = href if href.startswith("http") else f"https://www.albertacourts.ca{href}"
        ...
        court_file = ""
        file_match = re.search(r"\d{4}\s*[A-Z]{2,4}\s*\d+", text)
        if file_match:
        ...
        return ScraperResult(
            ...
            signal_text=f"Alberta class proceeding: {case_title}",
