...
class SCCScraper(BaseScraper):
    ...
    def _classify(self, text: str) -> list[str]:
        text = text.lower()
        areas = []
        if any(k in text for k in ["employment", "labour", "discrimination", "human rights"]):
            areas.append("employment")
        if any(k in text for k in ["contract", "negligence", "tort", "liability"]):
            areas.append("litigation")
        if any(k in text for k in ["criminal", "charter", "evidence"]):
            areas.append("litigation")
        if any(k in text for k in ["tax", "income", "assessment"]):
            areas.append("tax")
        if any(k in text for k in ["aboriginal", "indigenous", "treaty"]):
            areas.append("indigenous")
        if any(k in text for k in ["competition", "merger", "antitrust"]):
            areas.append("competition")
        if any(k in text for k in ["privacy", "data", "information"]):
            areas.append("privacy")
        if any(k in text for k in ["securities", "investment", "fraud"]):
            areas.append("securities")
