...
class ECCCScraper(BaseScraper):
    ...
    async def scrape(self) -> list[ScraperResult]:
        results = []
        try:
            resp = await self.get("https://www.canada.ca/en/environment-climate-change/news/rss.xml")
            if resp.status_code != 200: return results
            root = ET.fromstring(resp.text)
            for item in root.iter("item"):
                title = (item.findtext("title") or "").strip()
                title_lower = title.lower()
                if not any(k in title_lower for k in ["penalty", "conviction", "fine", "violation", "enforcement"]):
                    continue
                results.append(ScraperResult(
                    source_id=self.source_id, signal_type="regulatory_environmental_enforcement",
